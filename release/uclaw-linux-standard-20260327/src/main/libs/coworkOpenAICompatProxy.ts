...
export type OpenAICompatProxyTarget = 'local' | 'local';
...
function normalizeScheduledTaskWorkingDirectory(value: unknown): string {
  ...
  // local guest workspace roots are not valid host directories.
...
function resolveUpstreamAPIType(provider?: string): UpstreamAPIType {
  // 只有真正的 OpenAI (api.openai.com) 才用 Responses API
  // 第三方 OpenAI 兼容 API (SiliconFlow, Naga 等) 用 chat_completions
...
function isToolsUnsupportedError(errorMessage: string): boolean {
  ...
  return normalized.includes('does not support tools')
    ...
    || normalized.includes('unable to use tool')
...
function readRequestBody(req: http.IncomingMessage): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const decodeBody = (raw: Buffer): string => {
      ...
      // If strict UTF-8 succeeds, use it directly — no need to compare with GB18030.
      if (utf8Decoded) {
        return utf8Decoded;
      }

      // UTF-8 failed — on Windows, try GB18030 as fallback for system codepage.
      if (process.platform === 'win32') {
        ...
        if (gbDecoded) {
          console.warn('[CoworkProxy] Decoded request body using gb18030 fallback (UTF-8 failed)');
          return gbDecoded;
        }
      }
...
async function handleRequest(
  ...
  // SDK auxiliary endpoints are intentionally handled locally so they do not
  // participate in the real model communication path.
  ...
  // Short-circuit non-streaming probe/warmup requests from Claude Agent SDK.
  // The SDK sends a stream=false request before the real stream=true call;
  // hitting the upstream API for this wastes ~3 seconds per turn.
  if (!stream) {
    console.log(`[CoworkProxy] Short-circuit non-streaming probe (model=${upstreamRequest.model}, provider=${upstreamConfig.provider})`);
    writeJSON(res, 200, {
      id: `msg_probe_${Date.now()}`,
      type: 'message',
      role: 'assistant',
      content: [{ type: 'text', text: '' }],
      model: String(upstreamRequest.model || upstreamConfig.model || 'unknown'),
...
export function getCoworkOpenAICompatProxyBaseURL(target: OpenAICompatProxyTarget = 'local'): string | null {
  ...
  const host = target === 'local' ? LOCAL_HOST : local_HOST;
