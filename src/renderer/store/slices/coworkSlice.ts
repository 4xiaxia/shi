...
const coworkSlice = createSlice({
  ...
  reducers: {
    ...
    setLoadingSessionId(state, action: PayloadAction<string | null>) {
    ...
    setCurrentSession(state, action: PayloadAction<CoworkSession | null>) {
      ...
      if (action.payload) {
        ...
        if (!action.payload.id.startsWith('temp-')) {
          const { id, title, status, pinned, createdAt, updatedAt, agentRoleKey, modelId } = action.payload;
    ...
    addSession(state, action: PayloadAction<CoworkSession>) {
      const summary: CoworkSessionSummary = {
        ...
        systemPrompt: action.payload.systemPrompt,
        agentRoleKey: action.payload.agentRoleKey,
    ...
    addMessage(state, action: PayloadAction<{ sessionId: string; message: CoworkMessage }>) {
      ...
      if (state.currentSession?.id === sessionId) {
        ...
        if (!exists) {
          ...
          if (state.currentSession.historyMeta) {
            state.currentSession.historyMeta.loadedMessageCount += 1;
            state.currentSession.historyMeta.totalMessageCount += 1;
          }
        }
      }
    ...
    clearPendingPermissionsForSession(state, action: PayloadAction<{ sessionId: string }>) {
      state.pendingPermissions = state.pendingPermissions.filter(
        (permission) => permission.sessionId !== action.payload.sessionId
      );
    },
