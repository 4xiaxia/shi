...
let cachedBundledNodeRuntimePath: string | null = null;

function resolveBundledNodeRuntimePath(): string {
  if (!isBundledRuntime() || process.platform !== 'darwin') {
    return process.execPath;
  }

  try {
    const appName = app.getName();
    const frameworksDir = join(getRuntimeResourcesRoot(), '..', 'Frameworks');
    ...
    for (const helperApp of helperApps) {
      ...
      if (existsSync(helperExePath)) {
        coworkLog('INFO', 'resolveNodeShim', `Using bundled desktop helper runtime for node shim: ${helperExePath}`);
...
export function getBundledNodeRuntimePath(): string {
  if (!cachedBundledNodeRuntimePath) {
    cachedBundledNodeRuntimePath = resolveBundledNodeRuntimePath();
  }
  return cachedBundledNodeRuntimePath;
}

/**
 * Backward-compatible alias while de-Electronization is in progress.
 * New code should prefer getBundledNodeRuntimePath().
 */
export function getElectronNodeRuntimePath(): string {
  return getBundledNodeRuntimePath();
}
...
function getBundledGitBashCandidates(): string[] {
  const bundledRoots = isBundledRuntime()
    ? [join(getRuntimeResourcesRoot(), 'mingit')]
    : [
      join(__dirname_esm, '..', '..', 'resources', 'mingit'),
      join(getProjectRoot(), 'resources', 'mingit'),
    ];
...
function ensureBundledNodeShim(bundledNodeRuntimePath: string, npmBinDir?: string): string | null {
  try {
    const shimDir = join(app.getPath('userData'), 'cowork', 'bin');
    mkdirSync(shimDir, { recursive: true });
    coworkLog('INFO', 'resolveNodeShim', `Shim directory: ${shimDir}, bundledNodeRuntimePath: ${bundledNodeRuntimePath}, npmBinDir: ${npmBinDir || '(none)'}`);
...
function findWindowsGitBashEmergencyFallback(): string | null {
  const fallbacks = [
...
function applyPackagedEnvOverrides(env: Record<string, string | undefined>): void {
  ...
  if (isBundledRuntime() && !env.LOBSTERAI_ELECTRON_PATH) {
  ...
  if (process.platform === 'win32') {
    ...
    if (configuredBashPath && bashPath === configuredBashPath) {
      ...
      if (!configuredHealth.ok) {
        ...
        if (fallbackPath && fallbackPath !== configuredBashPath) {
          ...
          const emergencyFallback = existsSync(configuredBashPath)
            ? configuredBashPath
            : findWindowsGitBashEmergencyFallback();
          if (emergencyFallback) {
            coworkLog(
              'WARN',
              'resolveGitBash',
              `Using Windows git-bash emergency fallback without passing health check: ${emergencyFallback} (${configuredHealth.reason || 'unknown reason'})`
            );
            bashPath = emergencyFallback;
          } else {
    ...
    if (!bashPath) {
      const emergencyFallback = findWindowsGitBashEmergencyFallback();
      if (emergencyFallback) {
        coworkLog(
          'WARN',
          'resolveGitBash',
          `Using Windows git-bash emergency fallback without passing health check: ${emergencyFallback}`
        );
        bashPath = emergencyFallback;
      }
    }
  ...
  if (!isBundledRuntime()) {
    ...
    const devBinDir = getRuntimeAppPath('node_modules', '.bin');
  ...
  const resourcesPath = getRuntimeResourcesRoot();
  coworkLog('INFO', 'applyPackagedEnvOverrides', `Packaged mode: resourcesPath=${resourcesPath}`);

  // Create node/npx/npm shims that wrap the bundled desktop runtime as a Node.js runtime via
  ...
  if (shouldInjectShim) {
    const shimDir = ensureBundledNodeShim(bundledNodeRuntimePath, npmBinDir);
    if (shimDir) {
      env.PATH = [shimDir, env.PATH].filter(Boolean).join(delimiter);
      env.LOBSTERAI_NODE_SHIM_ACTIVE = '1';
      coworkLog('INFO', 'resolveNodeShim', `Injected bundled node/npx/npm shim PATH entry: ${shimDir}`);
    ...
    coworkLog('INFO', 'resolveNodeShim', 'System node/npx/npm detected; skipped bundled node shim injection');
  }
...
export function getSkillsRoot(): string {
  ...
  const candidates = [
    ...envRoots,
    getRuntimeAppPath('SKILLs'),
    join(getProjectRoot(), 'SKILLs'),
    join(__dirname_esm, '..', 'SKILLs'),
    join(__dirname_esm, '..', '..', 'SKILLs'),
  ];
  ...
  return getRuntimeAppPath('SKILLs');
}
 ...
 */
export async function getEnhancedEnv(target: OpenAICompatProxyTarget = 'local', agentRoleKey?: string): Promise<Record<string, string | undefined>> {
  const config = getCurrentApiConfig(target, agentRoleKey);
  const env = config
  ...
  if (process.platform === 'win32' || env.LOBSTERAI_NODE_SHIM_ACTIVE === '1') {
    env.LOBSTERAI_ELECTRON_PATH = getBundledNodeRuntimePath().replace(/\\/g, '/');
  } else {
...
export async function getEnhancedEnvWithTmpdir(
  ...
  const env = await getEnhancedEnv(target, agentRoleKey);
...
const COWORK_MODEL_PROBE_TIMEOUT_MS = 5000;
const COWORK_MODEL_PROBE_CACHE_TTL_MS = 600000;
const API_ERROR_SNIPPET_MAX_CHARS = 240;
const coworkModelProbeCache = new Map<string, {
  at: number;
  result: { ok: true; config?: { model: string } } | { ok: false; error: string };
}>();
...
export async function probeCoworkModelReadiness(
...
): Promise<{ ok: true; config?: { model: string } } | { ok: false; error: string }> {
  const { config, error } = resolveCurrentApiConfig('local', agentRoleKey, { dbOnly: true });
  if (!config) {
    return {
      ok: false,
  ...
  const cachedEntry = coworkModelProbeCache.get(cacheKey);
  if (cachedEntry && cacheNow - cachedEntry.at <= COWORK_MODEL_PROBE_CACHE_TTL_MS) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      coworkModelProbeCache.set(cacheKey, { at: cacheNow, result });
      return result;
    }

    const result = { ok: true, config: { model: config.model } } as const;
    coworkModelProbeCache.set(cacheKey, { at: cacheNow, result });
    return result;
  } catch (error) {
